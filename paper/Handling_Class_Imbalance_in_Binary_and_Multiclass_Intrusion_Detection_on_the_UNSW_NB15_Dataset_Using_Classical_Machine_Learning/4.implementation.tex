...
\subsection{Statistical Proof of Performance}
...
\subsubsection{Proof 1: Stability Analysis via Bootstrapping}
...
\textbf{Conclusion:} The non-overlapping confidence intervals between S0 ([0.865, 0.870]) and S1 ([0.900, 0.904]) provide \textbf{statistical proof} that the Cost-Sensitive strategy yields a genuine performance improvement distinct from noise.

\subsubsection{Proof 2: Strategy Ranking (Friedman Test)}
To prove that S1 and S2a consistently outperform S0 across the diverse attack landscape (10 classes), we conducted a Friedman Test on the per-class F1 scores.
\begin{itemize}
...
\end{itemize}
This empirically proves the specific "No Free Lunch" cost for this dataset: detecting Shellcode requires accepting a false positive rate that includes distinct subsets of Fuzzer traffic.
