...
\multirow{3}{*}{Logistic Regression} & C (regularization) & 1.0 \\
 ...
 & Max iterations & 1000 \\
\hline
\multirow{4}{*}{Random Forest} & n\_estimators & 300 \\
 & max\_depth & None \\
 & min\_samples\_split & 2 \\
 & min\_samples\_leaf & 1 \\
\hline
\multirow{4}{*}{XGBoost} & n\_estimators & 150 \\
 & learning\_rate & 0.05 \\
 & max\_depth & 15 \\
 & subsample & 0.85 \\
\hline
...
\subsection{Statistical Performance Validation}
To rigorously validate our findings and reject the null hypothesis that performance differences are due to random variance, we employed a \textbf{Statistical Validation Protocol} consisting of Parametric Bootstrapping ($n=1000$) and the Friedman Rank Test.

\subsubsection{Evidence 1: Stability Analysis via Bootstrapping}
We generated 95\% Confidence Intervals (CI) for the Macro-F1 score by resampling the test set confusion matrices. As presented in Table~\ref{tab:ci_results}, the error margins ($\pm \delta$) demonstrate the high precision of our estimates.

\begin{table}[h]
\centering
\caption{Bootstrapped Performance Estimates (Mean $\pm$ 95\% Margin of Error)}
\label{tab:ci_results}
\begin{tabular}{|l|l|c|l|c|}
\hline
\textbf{Model} & \textbf{Strategy} & \textbf{Metric} & \textbf{Mean Estimate} & \textbf{Significance} \\
\hline
XGB & S0 (Baseline) & Macro-F1 & $0.868 \pm 0.002$ & Reference \\
\textbf{XGB} & \textbf{S1 (Weight)} & \textbf{Macro-F1} & $\mathbf{0.902 \pm 0.002}$ & \textbf{Significant} \\
XGB & S2a (ROS) & Macro-F1 & $0.897 \pm 0.002$ & Significant \\
\hline
\end{tabular}
\end{table}

\textbf{Conclusion:} The non-overlapping confidence intervals between S0 ([0.865, 0.870]) and S1 ([0.900, 0.904]) provide \textbf{statistical evidence} that the Cost-Sensitive strategy yields a genuine performance improvement distinct from noise.

\subsubsection{Evidence 2: Strategy Ranking (Friedman Test)}
...
\begin{itemize}
    \item \textbf{Null Hypothesis ($H_0$)}: There is no difference between strategies.
    \item \textbf{Result}: $p = 0.00026$ ($p \ll 0.05$).
\end{itemize}
We reject $H_0$. The Nemenyi post-hoc visualization (Figure~\ref{fig:cd_diagram}) statistically confirms that both balanced strategies dominate the baseline.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{figures/rank_comparison.png}
\caption{Critical Difference Rankings (Lower is Better). The statistical gap between S0 (Rank ~8.0) and S1/S2a (Rank ~1.5) provides evidence of systematic superiority across classes.}
\label{fig:cd_diagram}
\end{figure}

\subsection{Forensic Evidence of Rare-Class Dynamics}
Beyond aggregate statistics, we present forensic evidence identifying the structural causes of rare-class failures.

\subsubsection{Evidence 1: The Analysis-Backdoor Confound}
The \textit{Analysis} category consistently underperforms. A forensic inspection of the XGB-S1 confusion matrix reveals a deterministic error pattern:
\begin{equation}
\text{Confusion Ratio} = \frac{P(\text{Pred Backdoor} | \text{True Analysis})}{P(\text{Pred Analysis} | \text{True Analysis})} = 2.37
\end{equation}
\textbf{Finding:} The model is $2.37\times$ more likely to misclassify an \textit{Analysis} sample as \textit{Backdoor} than to correctly identify it. This provides \textbf{evidence} of topological overlap in the feature space, proving that hyperparameter tuning cannot resolve this distinction without new features.

\subsubsection{Evidence 2: The Shellcode Sink (Recall-Precision Trade-off)}
We present the trade-off inherent in detecting \textit{Shellcode} as a quantifiable cost.
\begin{itemize}
    \item \textbf{Recall Gain}: S1 increases detection from 0/378 (S0) to 356/378.
    \item \textbf{Precision Cost}: This introduces 300+ false positives from \textit{Fuzzers} and \textit{Normal} traffic.
\end{itemize}
...
\subsection{Feature-Level Classification Results}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figures/radar_multi_xgb.png}
\caption{Radar Chart Comparison: XGBoost Performance Across Metrics (S0 vs S1 vs S2a). The balanced strategies (S1, S2a) show superior G-Mean and Macro-F1 coverage compared to the skewed baseline (S0).}
\label{fig:radar_multi}
\end{figure}
...
\hline
\textbf{Class} & \textbf{Support} & \textbf{LR-S0 Recall} & \textbf{XGB-S1 Recall} & \textbf{Outcome} \\
\hline
Worms & 44 & 0.000 & 0.841 & \textbf{Enabled} \\
Shellcode & 378 & 0.000 & 0.942 & \textbf{Enabled} \\
Backdoor & 583 & 0.000 & 0.606 & \textbf{Enabled} \\
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\linewidth]{figures/rare_class_f1_comparison.png}
\caption{Rare Class F1-Score Comparison. The baseline model (Blue) completely fails to detect rare classes. Imbalance handling (Green/Red) restores visibility for Worms and Shellcode.}
\label{fig:rare_class_f1}
\end{figure}
